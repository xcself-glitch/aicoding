#!/usr/bin/env python3
"""
ETHUSDT 实时交易监控 - 带飞书通知
使用 Gate.io API 真实数据
参考 stock-monitor-pro 消息发送方式
"""
...
# 导入飞书通知模块
from scripts.eth_notify_feishu import send_trading_signal, send_market_summary
...
def main():
    ...
    try:
        ...
        if signal.type in [SignalType.LONG, SignalType.SHORT]:
            ...
            expected_return = profit_cfg['leveraged_return']
            
            print(f"\n{'='*60}")
            print(f"📢 🚨 ETHUSDT {direction}信号 - 正在发送飞书通知...")
            print(f"{'='*60}")
            ...
            send_trading_signal(
                ...
                daily_high=ticker.high_24h,
                daily_low=ticker.low_24h,
            ...
            # 无交易信号，发送市场状态（可选，可以取消以减少消息量）
            print(f"\n⏸️ 当前状态: {signal.reason}")
            
            # 注释掉下面这行如果不需要定期发送市场状态
            # send_market_summary(
            #     price=current_price,
            #     change_pct=ticker.change_percentage,
            #     daily_high=ticker.high_24h,
