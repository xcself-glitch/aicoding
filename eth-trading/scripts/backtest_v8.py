...
class OptimizedSignalGenerator:
    ...
    def generate_signal(self, opens, highs, lows, closes, volumes, bar_idx):
        if bar_idx - self.last_bar < 4:
        ...
        if trend == "uptrend":
            ema10 = self.calculate_ema(closes, 10)
            near_ema = 0.003 < (ema10[-1] - current_price) / current_price < 0.015
            
            score = 0
            if near_ema and rsi < 65 and macd_signal.trend == TrendDirection.UP:
                score += 30
            ...
            # 下轨反弹
            if (current_price - lower[-1]) / current_price < 0.008:
                if macd_signal.trend == TrendDirection.UP:
                    score += 30
            
            if score >= 50:
                long_setup = f"做多得分{score}"
        
        # 做空条件 - 只在downtrend
        if trend == "downtrend":
            ema10 = self.calculate_ema(closes, 10)
            near_ema = 0.003 < (current_price - ema10[-1]) / current_price < 0.015
            
            score = 0
            if near_ema and rsi > 35 and macd_signal.trend == TrendDirection.DOWN:
                score += 30
            ...
            # 上轨回落
            if (upper[-1] - current_price) / current_price < 0.008:
                if macd_signal.trend == TrendDirection.DOWN:
                    score += 30
            
            if score >= 50:
                short_setup = f"做空得分{score}"
        
        if long_setup:
            self.last_bar = bar_idx
            return "LONG", support, resistance, long_setup
        elif short_setup:
            self.last_bar = bar_idx
            return "SHORT", support, resistance, short_setup
...
class Backtester:
    def __init__(self, initial_capital: float = 10000.0):
        ...
        # 仓位配置
        self.first_pct = 0.20   # 首仓20%
        self.second_pct = 0.20  # 二仓20%
        self.third_pct = 0.15   # 三仓15%
        self.max_exposure = 0.55  # 最大55%
            ...
            if position:
                ...
                if pnl_pct > 10 and position.trail_sl == 0:
                    if position.direction == "LONG":
                        position.trail_sl = position.avg_price * 1.02  # 保2%利润
                    else:
                        position.trail_sl = position.avg_price * 0.98
                ...
                # 补仓逻辑
                    ...
                    # 浮亏达到3%补仓
                    if pnl_pct < -3:
                        current_exposure = sum(p*q for p,q in position.entries) / self.initial_capital
                        ...
                        if add_pct > 0:
                            ...
                            if position.direction == "LONG":
                                position.sl_price = position.avg_price - atr * 1.5
                            else:
                                position.sl_price = position.avg_price + atr * 1.5
                            
                            total_exposure = sum(p*q for p,q in position.entries) / self.initial_capital
                            print(f"   ➕ 补仓#{position.entry_count} 价格:{current_price:.2f} 仓位:{total_exposure*100:.0f}%")
            
            else:
                direction, support, resistance, setup = generator.generate_signal(
                ...
                if direction:
                    ...
                    print(f"   💰 入场:{current_price:.2f} 首仓20% 止损:{sl:.2f}({risk_pct:.2f}%) 止盈:{tp:.2f}({reward_pct:.2f}%)")
...
def print_report(result, initial_capital, days):
    ...
    if add_trades:
        print(f"\n📝 补仓统计: {len(add_trades)}次补仓")
    
    if result.trades:
        print(f"\n📋 全部交易明细:")
        print("-"*80)
        print(f"{'时间':<14} {'方向':<5} {'入场':<18} {'出场':<10} {'收益':<10} {'原因':<12}")
        print("-"*80)
        ...
        for trade in result.trades:
            ...
            print(f"{entry_time:<14} {direction:<5} {entry_info:<18} {trade.exit_price:<10.2f} {pnl_pct*100:+.1f}%      {trade.exit_reason:<12}")
...
def main():
    print("🚀 ETHUSDT V8策略回测 - V4优化版")
    print("="*80)
    print("🎯 核心特性:")
    print("   1️⃣ 基于V4趋势突破策略(胜率50%，周收益23%)")
    print("   2️⃣ 分批建仓: 20%+20%+15%，最大55%仓位")
    print("   3️⃣ 移动止损: 盈利10%启动，保护利润")
    print("   4️⃣ 补仓条件: 浮亏3%后补仓")
    print("="*80)
