...
class V12SignalGenerator:
    ...
    def get_trend(self, closes, highs, lows):
        """判断趋势强度和方向"""
        if len(closes) < 50:
            return "neutral", 0
        ...
        if bull_score >= 70:
            return "strong_bull", bull_score
        elif bull_score >= 40:
            return "bull", bull_score
        elif bear_score >= 70:
            return "strong_bear", bear_score
        elif bear_score >= 40:
    ...
    def generate_signal(self, opens, highs, lows, closes, volumes, current_time):
        ...
        trend, strength = self.get_trend(closes, highs, lows)
        ...
        if trend == "strong_bull":
            ...
            if 0.003 < pullback < 0.015 and rsi < 60:
        ...
        # ===== 做空 - 只做强空头趋势 =====
            ...
            if 0.003 < bounce < 0.015 and rsi > 40:
            ...
            if position:
                ...
                if pnl_pct > 15 and position.trail_sl == 0:
                    if position.direction == "LONG":
                        position.trail_sl = position.avg_price * 1.08
                    else:
                        position.trail_sl = position.avg_price * 0.92
                ...
                if signal_type in [SignalType.LONG, SignalType.SHORT]:
                    ...
                    # 止盈止损 - 1%止损，3%止盈，盈亏比3:1
                    if is_long:
                        sl = support * 0.995
                        risk = current_price - sl
                        tp = current_price + risk * 3
                        if risk / current_price > 0.012:
                            sl = current_price * 0.988
                            risk = current_price * 0.012
                            tp = current_price + risk * 3
                    else:
                        sl = resistance * 1.005
...
def print_report(result, initial_capital, days):
    print("\n" + "="*80)
    print("📊 ETHUSDT V12策略回测报告 (平衡版)")
    print("="*80)
    print("🎯 目标: 周收益50%+ | 胜率60%+")
    print("🆕 优化: 只做强趋势(strength>=70) + 1.2%止损 + 移动止损")
    print("-"*80)
...
def main():
    print("🚀 ETHUSDT V12策略回测 - 平衡版")
    print("="*80)
    print("🎯 V12优化点:")
    print("   1️⃣ 只做强趋势(strength>=70)，过滤震荡和弱趋势")
    print("   2️⃣ 止损放宽到1.2%，减少噪音止损")
    print("   3️⃣ 移动止损盈利15%启动，保8%利润")
    print("   4️⃣ 盈亏比3:1")
    print("="*80)
